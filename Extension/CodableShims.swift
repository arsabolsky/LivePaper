// CodableShims.swift — Codable structs matching WallpaperTypes' NSKeyedArchiver encoding.
// Copied VERBATIM from PhospheneExtension/CodableShims.swift — these structs must reproduce
// Apple's private WallpaperTypes encoding byte-for-byte, so the field names, key strategies,
// and enum encodings are OS-level and must NOT be renamed or "Sakura"-prefixed.
//
// Mechanism: we archive a ShimViewModelsXPC with NSKeyedArchiver, swap the archived class
// name to WallpaperSettingsViewModelsXPC, and unarchive — the real private class's
// init(coder:) then decodes our shim bytes via its own Codable conformance. This is the
// only viable way to construct the private WallpaperSettingsViewModelsXPC from outside.
